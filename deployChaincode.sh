...
setGlobalsForPeer1Org1() {
    ...
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG1_CA
    export CORE_PEER_MSPCONFIGPATH=${PWD}/artifacts/channel/crypto-config/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp
    export CORE_PEER_ADDRESS=localhost:3061
...
getBlock() {
    ...
    peer channel getinfo  -c mychannel -o localhost:3050 \
        --ordererTLSHostnameOverride orderer.fabcar.example.com --tls \
        --cafile $ORDERER_CA
}
...
chaincodeInvoke() {
    # setGlobalsForPeer0Org1
    # peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.fabcar.example.com\
